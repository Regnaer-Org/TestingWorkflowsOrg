...
PARENT_MILESTONE_NUMBER = os.environ.get("PARENT_MILESTONE_NUMBER") # String representation
...
if not PARENT_ISSUE_NODE_ID:
    ...
    sys.exit(1)
if PARENT_MILESTONE_NUMBER is None: # Could be explicitly empty if unmilestoned, handle later
    print("Info: PARENT_MILESTONE_NUMBER is not set (likely unmilestoned event).")
    # Allow script to continue, it will effectively clear milestones on sub-issues
elif not PARENT_MILESTONE_NUMBER.isdigit():
     print(f"Error: PARENT_MILESTONE_NUMBER ({PARENT_MILESTONE_NUMBER}) is not a valid number.", file=sys.stderr)
     sys.exit(1)
  ...
  node(id: $parentId) {
    ... on Issue {
      ...
      subIssues(first: 100) { # Fetch up to 100 sub-issues
        ...
        # pageInfo { hasNextPage endCursor } # Add pagination if >100 sub-issues expected
      }
    }
  }
}
...
def rest_patch_request(endpoint, payload):
    ...
    try:
        ...
        # Check for non-fatal errors like 404 (issue might be deleted) or 422 (validation)
        if 400 <= response.status_code < 500:
             ...
             return False # Indicate potential issue, but don't stop script
        response.raise_for_status() # Raise exception for 5xx or unexpected 4xx
...
print(f"Parent Milestone Number: {PARENT_MILESTONE_NUMBER if PARENT_MILESTONE_NUMBER is not None else 'None (Unmilestoned)'}")
...
# Determine milestone value for REST API (null for unmilestoned)
milestone_payload_value = int(PARENT_MILESTONE_NUMBER) if PARENT_MILESTONE_NUMBER is not None else None
...
for sub in sub_issues:
    ...
    rest_payload = {"milestone": milestone_payload_value}
    ...
    if add_data and add_data.get("addProjectV2ItemById", {}).get("item"):
        ...
        # Note: We don't track a separate count for *newly* added vs already present
    else:
        print(f"    Warning: Failed to add/confirm sub-issue #{sub_issue_number} in project {PROJECT_ID}.")
        # Don't increment error_count here as it might just mean permissions issue or already added, but log it.
...
print(f"Sub-issues Add-to-Project Attempted (Success/Already Present): {len(sub_issues) - error_count}") # Approximation
